-- ============================================================
-- Production Order Contract Reconciliation
--
-- The live public.orders table is the canonical source of truth for this
-- deployment. This migration adds the missing application-contract columns,
-- restores relational order tables with bigint order IDs, and installs a
-- create_order_with_items() implementation that writes both orders.items and
-- order_items atomically.
--
-- No existing columns are dropped or renamed.
-- ============================================================

-- 1. Reconcile the live orders table with the application contract.
...
-- 2. Restore the relational order-items table using the live bigint order PK.
...
-- 3. Restore timeline and shipment relations with bigint order IDs.
...
-- 4. Document numbering used by the V2 service.
...
-- 5. RLS policies are guarded with DO blocks because PostgreSQL does not
...
-- 6. Atomically create the canonical order row and relational item snapshots.
-- The existing orders.items JSONB representation is retained for fulfillment;
-- order_items is populated in the same transaction for Order V2/admin reads.
